...
<body>
    ...
    <?php
        ...
        if ($userid) {
            // Select the flight_id based on the provided user_id
            $selectQuery = "SELECT flight_id FROM pending_flights_users WHERE user_id = ?";
            $stmt = $conn->prepare($selectQuery);
            $stmt->bind_param("i", $userid);
            $stmt->execute();
            $stmt->bind_result($flightId);

            // Fetch the flight_id
            if ($stmt->fetch()) {
                echo "Flight ID for User ID {$userid}: $flightId";
            } else {
                echo "No pending flight found for User ID: $userid";
            }
        ...
        if ($result !== false && $result->num_rows > 0) {
            ...
            echo '<form action="/Flight-Booking-Website/Backend/final_reserve.php" method="get">';
            echo '<input type="hidden" name="userid" value="' . $userid . '">';
            echo '<input type="hidden" name="flightid" value="' . $flightId . '">';
            echo '<button type="submit">Finalize Booking</button>';
            echo '</form>';
        } else {
